...
import { generateCadreAssistantReply } from '@/lib/ai';
...
export async function POST(request: Request) {
  ...
  const reply = await generateCadreAssistantReply(payload.scene, payload.prompt);
  ...
  await prisma.cadreConversation.createMany({
    data: [
      ...
      },
      {
        scene: payload.scene,
        role: 'assistant',
        stage: reply.source === 'glm' ? '模型建议' : '本地建议',
  ...
  const conversations = await prisma.cadreConversation.findMany({
